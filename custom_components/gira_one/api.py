...
class GiraApiClient:
    ...
    async def _request(
        ...
        if requires_auth:
            if is_registration:
                if not self._username or not self._password:
                    ...
                    raise GiraApiAuthError("Username or password missing for registration.")
                ...
                _LOGGER.error("Token not available for authenticated request to %s", path)
                raise GiraApiAuthError(f"Token missing for {path}")

        _LOGGER.debug("Request: %s %s (params=%s, json=%s)", method, url, params, json_data)

        try:
            async with async_timeout.timeout(15): # Increased timeout for potential slow devices
            ...
            _LOGGER.error("SSL Certificate Verification failed for %s: %s. Ensure Gira device uses a valid cert or disable verification if intended for client talking to Gira.", url, err)
        ...
        try:
            response_data = await response.json() if response.content_length and response.content_length > 0 else {}
        except aiohttp.ContentTypeError:
            # Handle cases like 204 No Content
            if status_code == 204:
                response_data = {}
            else:
                _LOGGER.warning("Non-JSON response from %s: %s", url, await response.text())
        ...
        if not (200 <= status_code < 300):
            ...
            _LOGGER.error(
                ...
                url, status_code, error_code, error_message, response_data
            )
            if status_code == 401: # Unauthorized
                if error_code == "invalidAuth": # Missing or invalid authentication.
                    raise GiraApiAuthError(f"Authentication failed: {error_message} (Code: {error_code})")
                elif error_code == "invalidToken" and not is_registration: # Missing or invalid token as URL parameter.
                     # Token might have been invalidated on the server
                    self._token = None # Clear stale token
                    raise GiraApiAuthError(f"Invalid token: {error_message} (Code: {error_code})")
            elif status_code == 423: # Locked
                 raise GiraApiRequestError(f"Device locked: {error_message} (Code: {error_code}, Status: {status_code})")
    ...
    async def check_api_availability(self) -> Dict[str, Any]:
        ...
        status, data = await self._request("GET", f"/{API_VERSION}/", requires_auth=False)
        if status == 200 and data.get("info") == "GDS-REST-API": #
            _LOGGER.info("Gira API is available: %s", data)
            return data
        raise GiraApiConnectionError(f"API availability check failed. Status: {status}, Data: {data}")
    ...
    async def register_client(self, client_id: str) -> str:
        ...
        try:
            ...
            status, data = await self._request("POST", f"/{API_VERSION}/clients", json_data=payload, requires_auth=True, is_registration=True)
        except GiraApiAuthError as e: #
            _LOGGER.error("Client registration failed due to authentication error: %s", e)
            raise
        ...
        if status == 201 and "token" in data: #
            ...
            return self._token
        _LOGGER.error("Failed to register client or get token. Status: %s, Data: %s", status, data)
        raise GiraApiRequestError(f"Client registration failed. Status: {status}, Response: {data}")
    ...
    async def unregister_client(self) -> None:
        ...
        try:
            ...
            async with async_timeout.timeout(10):
                response = await self._session.delete(url, headers=headers) # No params, no json_data
            ...
            if response.status == 204: # No Content on success
                ...
                _LOGGER.warning("Failed to unregister client: Invalid token (already unregistered?).")
                self._token = None # Assume token is no longer valid
            else:
                error_text = await response.text()
                _LOGGER.error("Failed to unregister client. Status: %s, Response: %s", response.status, error_text)
                ...
                try:
                    ...
                    raise GiraApiRequestError(f"Client unregistration failed. Status: {response.status}, Code: {error_code}, Msg: {error_message}")
                except Exception: # Not JSON or other parsing error
                     raise GiraApiRequestError(f"Client unregistration failed. Status: {response.status}, Response: {error_text}")

        except (aiohttp.ClientError, asyncio.TimeoutError) as err:
            _LOGGER.error("Error during client unregistration request: %s", err)
            raise GiraApiConnectionError(f"Unregistration request failed: {err}") from err
    ...
    async def get_ui_config(self) -> Dict[str, Any]:
        ...
        status, data = await self._request("GET", f"/{API_VERSION}/uiconfig", params=params)
        if status == 200:
            return data
        raise GiraApiRequestError(f"Failed to get UI config. Status: {status}, Data: {data}")
    ...
    async def get_value(self, uid: str) -> Dict[str, Any]:
        ...
        if status == 200 and "values" in data:
            return data
        raise GiraApiRequestError(f"Failed to get value for UID {uid}. Status: {status}, Data: {data}")
    ...
    async def set_value(self, uid: str, value: Any) -> bool:
        ...
        payload = {"value": value} #
        # PUT /api/values/<uid>
        status, _ = await self._request("PUT", f"/{API_VERSION}/values/{uid}", json_data=payload)
    ...
    async def set_multiple_values(self, values_payload: list[dict[str, Any]]) -> bool:
        ...
        # PUT /api/values
        status, _ = await self._request("PUT", f"/{API_VERSION}/values", json_data=payload)
        return status == 200 # PUT returns 200 OK on success

    async def register_callbacks(self, service_callback_url: str, value_callback_url: str) -> bool:
        ...
        status, data = await self._request("POST", f"/{API_VERSION}/clients/{self._token}/callbacks", json_data=payload, requires_auth=False) # Token is in path
        if status == 200: #
        ...
        if error_code == "callbackTestFailed":
            _LOGGER.error("Callback registration failed: Gira API could not reach or validate callback URLs. %s", error_message)
        elif error_code == "unprocessable" and "HTTPS" in error_message:
            _LOGGER.error("Callback registration failed: Callback URLs must use HTTPS. %s", error_message)
        else:
            _LOGGER.error("Failed to register callbacks. Status: %s, Code: %s, Message: %s", status, error_code, error_message)
        raise GiraApiRequestError(f"Callback registration failed: {error_message} (Status: {status}, Code: {error_code})")
    ...
    async def remove_callbacks(self) -> bool:
        ...
        status, data = await self._request("DELETE", f"/{API_VERSION}/clients/{self._token}/callbacks", requires_auth=False) # Token is in path
        if status == 200: #
        ...
        _LOGGER.error("Failed to remove callbacks. Status: %s, Code: %s, Message: %s", status, error_code, error_message)
