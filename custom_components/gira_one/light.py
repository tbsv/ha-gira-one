"""Platform for Gira IoT Light entities."""
import logging
...
async def async_setup_entry(
    ...
    for function in gira_functions:
        ...
        if platform == LIGHT:
            ...
            _LOGGER.info("Adding Gira Light: %s (UID: %s)", function.get("displayName"), function.get("uid"))
    ...
    def _async_reload_entities():
        _LOGGER.info("Gira UI config changed, attempting to reload light entities for %s.", config_entry.entry_id)
...
class GiraLight(LightEntity):
    ...
    def __init__(
        ...
        self._attr_name = function_data.get("displayName", f"Gira Light {self._attr_unique_id}")
    ...
    def _update_supported_color_modes_and_features(self) -> None:
        ...
        if DP_BRIGHTNESS in self._data_points:
            supported_color_modes.add(ColorMode.BRIGHTNESS) # Can be on/off + brightness
            supported_features |= LightEntityFeature.TRANSITION # Assuming Gira might handle transitions

        if DP_COLOR_TEMPERATURE in self._data_points:
            supported_color_modes.add(ColorMode.COLOR_TEMP)

        if DP_RED in self._data_points and DP_GREEN in self._data_points and DP_BLUE in self._data_points:
        ...
        if len(supported_color_modes) > 1 and ColorMode.ONOFF in supported_color_modes:
            if ColorMode.BRIGHTNESS in supported_color_modes or \
               ColorMode.COLOR_TEMP in supported_color_modes or \
               ColorMode.HS in supported_color_modes or \
               ColorMode.RGB in supported_color_modes or \
               ColorMode.RGBW in supported_color_modes:
                supported_color_modes.remove(ColorMode.ONOFF)

        if not supported_color_modes: # Fallback if somehow none detected
            _LOGGER.warning("No color modes detected for light %s, defaulting to ON/OFF", self.name)
            supported_color_modes.add(ColorMode.ONOFF)
        ...
        _LOGGER.debug("Light %s: Supported Modes: %s, Current Mode: %s, Features: %s",
                      self.name, self._attr_supported_color_modes, self._attr_color_mode, self._attr_supported_features)
    ...
    async def async_added_to_hass(self) -> None:
        ...
        try:
            ...
            _LOGGER.error("Error fetching initial state for light %s (UID: %s): %s", self.name, self.unique_id, e)


    @callback
    def _handle_value_update(self, dp_uid: str, value: Any) -> None:
        """Handle a value update from the Gira callback."""
        _LOGGER.debug("Light %s received value update for DP UID %s: %s", self.name, dp_uid, value)
    ...
    def _update_state_from_value(self, dp_uid_updated: str, value: Any) -> bool:
        ...
        for dp_name, dp_info in self._data_points.items():
            if dp_info["uid"] == dp_uid_updated:
                _LOGGER.debug("Matching DP UID %s with DP Name %s for light %s", dp_uid_updated, dp_name, self.name)
                parsed_value = self._parse_value(value, dp_name)
                if dp_name == DP_ON_OFF:
                    self._attr_is_on = bool(int(parsed_value))
                    if self._attr_is_on != original_is_on: changed = True
                ...
                # For now, just logging.
                    ...
                    _LOGGER.debug("Color component %s updated to %s. HS/RGB conversion needed.", dp_name, parsed_value)
                ...
                if changed:
                    _LOGGER.debug("State changed for light %s: OnOff=%s, Brightness=%s, ColorTemp=%s",
                                  self.name, self._attr_is_on, self._attr_brightness, self._attr_color_temp_kelvin)
                return changed # Found and processed the data point

        _LOGGER.warning("DP UID %s not found in known data points for light %s", dp_uid_updated, self.name)
        return False
    ...
    def _parse_value(self, value: Any, dp_name: str) -> Any:
        ...
        try:
            if dp_name in [DP_ON_OFF]:
                return int(value)
            if dp_name in [DP_BRIGHTNESS, DP_RED, DP_GREEN, DP_BLUE, DP_WHITE]: # Percentages
                return float(value) # Keep as 0-100 for now, convert to 0-255 later
            if dp_name in [DP_COLOR_TEMPERATURE]: # Float
                return float(value)
        except (ValueError, TypeError) as e:
            _LOGGER.warning("Could not parse value '%s' for data point %s: %s", value, dp_name, e)
            return None
        return value
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        # Color Temperature
        if ATTR_COLOR_TEMP_KELVIN in kwargs and DP_COLOR_TEMPERATURE in self._data_points:
        ...
        if ATTR_HS_COLOR in kwargs and \
           DP_RED in self._data_points and \
           DP_GREEN in self._data_points and \
           DP_BLUE in self._data_points:
            ...
            payloads.append({"uid": self._data_points[DP_GREEN]["uid"], "value": gira_g})
            ...
            if (ATTR_WHITE in kwargs or ATTR_RGB_COLOR in kwargs) and \
               DP_WHITE in self._data_points and \
               self.supported_color_modes and ColorMode.RGBW in self.supported_color_modes:
                # If ATTR_WHITE is present, it's for the W channel of RGBW (0-255)
                # If ATTR_RGBW_COLOR is used, the 4th component is W
                ha_white_val = kwargs.get(ATTR_WHITE) # Could come from ATTR_WHITE directly
                if ATTR_RGB_COLOR in kwargs and len(kwargs[ATTR_RGB_COLOR]) == 4 : # Check if it's RGBW
                ...
                if ha_white_val is not None:
                    ...
                    payloads.append({"uid": self._data_points[DP_WHITE]["uid"], "value": gira_white})


        if payloads:
            _LOGGER.debug("Setting light %s with payloads: %s", self.name, payloads)
            try:
                if len(payloads) == 1 and payloads[0]["uid"] == on_off_dp_uid and payloads[0]["value"] == 1 and len(kwargs)==0 :
