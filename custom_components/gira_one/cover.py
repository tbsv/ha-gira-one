...
async def async_setup_entry(
    ...
    api_client: GiraApiClient = hass.data[DOMAIN][config_entry.entry_id][DATA_API_CLIENT]
    ...
    for function_data in gira_functions:
        ...
        if platform == COVER:
            ...
            _LOGGER.info("Adding Gira Cover: %s (UID: %s)", function_data.get("displayName"), function_data.get("uid"))
    ...
    def _async_reload_entities():
        _LOGGER.info("Gira UI config changed, cover entities might need reload for %s.", config_entry.entry_id)
        # Full reload is typically managed by __init__.py from service callback.

    config_entry.async_on_unload(
        async_dispatcher_connect(
            hass, f"{SIGNAL_UI_CONFIG_CHANGED}_{config_entry.entry_id}", _async_reload_entities
        )
    )
...
class GiraCover(CoverEntity):
    ...
    def __init__(
        ...
        self._attr_name = function_data.get("displayName", f"Gira Cover {self._attr_unique_id}")
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, self.unique_id)},
            name=self.name,
            manufacturer="Gira",
            model=function_data.get("channelType"), # e.g., de.gira.schema.channels.BlindWithPos
        ...
        self._attr_current_cover_position: Optional[int] = None # 0 (open) to 100 (closed)
        self._attr_current_cover_tilt_position: Optional[int] = None # 0 (open) to 100 (closed)
        self._attr_is_moving: bool = False
        self._attr_is_opening: bool = False # Requires knowing direction if _attr_is_moving is true
        ...
        _LOGGER.debug("Initialized GiraCover %s with DPs: %s, Features: %s",
                      self.name, self._data_points.keys(), self.supported_features)
    ...
    def _update_supported_features(self) -> None:
        ...
        # Position control
        if DP_POSITION in self._data_points and self._data_points[DP_POSITION].get("canWrite", False): #
            features |= CoverEntityFeature.SET_POSITION

        # Tilt control
        if DP_SLAT_POSITION in self._data_points and self._data_points[DP_SLAT_POSITION].get("canWrite", False): #
            features |= CoverEntityFeature.OPEN_TILT | CoverEntityFeature.CLOSE_TILT | CoverEntityFeature.SET_TILT_POSITION
    ...
    async def _fetch_initial_state(self) -> None:
        ...
        try:
            _LOGGER.debug("Requesting initial state for cover function UID %s", self.unique_id)
            data = await self._api.get_value(self.unique_id) # Get all DPs for this function
            if data and "values" in data:
                for dp_value_info in data["values"]:
                    self._update_state_from_value(dp_value_info["uid"], dp_value_info["value"])
                self.async_write_ha_state()
                _LOGGER.debug("Initial state updated for cover %s", self.name)
        except Exception as e:
            _LOGGER.error("Error fetching initial state for cover %s (UID: %s): %s", self.name, self.unique_id, e)
            ...
            if dp_info["uid"] == dp_uid_updated:
                try:
                    if dp_name == DP_POSITION: #
                        parsed_value = int(float(value)) # Gira 0-100, HA 0-100 (0=open, 100=closed)
                        ...
                        if self._attr_is_moving != is_moving_now:
                            ...
                            if not is_moving_now: # Just stopped
                                ...
                                self._attr_is_opening = self._last_known_direction_is_opening
                                self._attr_is_closing = not self._last_known_direction_is_opening
                    # Up-Down and Step-Up-Down are write-only according to
                    # So no state updates from them directly.

                except (ValueError, TypeError) as e:
                    _LOGGER.warning("Could not parse value '%s' for cover DP %s (%s): %s", value, dp_name, dp_uid_updated, e)
                    return False

                if changed:
                    _LOGGER.debug(
                        "Cover %s state change: Position=%s, Tilt=%s, Moving=%s, IsClosed=%s",
                        self.name, self._attr_current_cover_position, self._attr_current_cover_tilt_position,
                        self._attr_is_moving, self._attr_is_closed
                    )
                return changed
        return False
    ...
    async def _send_command(self, dp_name: str, value: Any) -> None:
        ...
        _LOGGER.debug("Cover %s: Sending to DP '%s' (UID %s) value '%s'", self.name, dp_name, dp_uid, value)
        try:
            await self._api.set_value(dp_uid, value)
        except Exception as e:
            _LOGGER.error("Cover %s: Error sending command to DP '%s': %s", self.name, dp_name, e)

    async def async_open_cover(self, **kwargs: Any) -> None:
        """Open the cover."""
        _LOGGER.debug("Cover %s: Opening (current pos: %s)", self.name, self._attr_current_cover_position)
        self._attr_is_opening = True # Optimistic
        self._attr_is_closing = False
        self._last_known_direction_is_opening = True
        if self.supported_features & CoverEntityFeature.SET_POSITION and DP_POSITION in self._data_points:
    ...
    async def async_close_cover(self, **kwargs: Any) -> None:
        """Close the cover."""
        _LOGGER.debug("Cover %s: Closing (current pos: %s)", self.name, self._attr_current_cover_position)
    ...
    async def async_stop_cover(self, **kwargs: Any) -> None:
        ...
        if self.supported_features & CoverEntityFeature.SET_POSITION and \
           DP_POSITION in self._data_points and \
           self._attr_current_cover_position is not None:
            _LOGGER.debug("Stopping cover %s by setting current position %s", self.name, self._attr_current_cover_position)
            ...
            _LOGGER.warning("Cover %s: Stop functionality for Up-Down DP is ambiguous. Attempting a common stop value (e.g., if 'Up-Down' also accepts a stop command). This might not work.", self.name)
            ...
            if DP_STEP_UP_DOWN in self._data_points:
                 ...
                 _LOGGER.info("Cover %s: Stop via Up-Down is not clearly defined by Gira API docs. Position based stop is preferred.", self.name)
            else:
                 _LOGGER.info("Cover %s: No clear stop mechanism if SET_POSITION is not available.", self.name)
    ...
    async def async_stop_cover_tilt(self, **kwargs: Any) -> None:
        ...
        if self.supported_features & CoverEntityFeature.SET_TILT_POSITION and \
           DP_SLAT_POSITION in self._data_points and \
           self._attr_current_cover_tilt_position is not None:
            await self._send_command(DP_SLAT_POSITION, self._attr_current_cover_tilt_position)
        else:
            _LOGGER.warning("Cover %s: Cannot stop tilt without SET_TILT_POSITION support or known current tilt.", self.name)
